...
Set-Content -Path (Join-Path $repository "README.md") -Value "m3-demo" -Encoding UTF8
git -C $repository -c user.email=m3-demo@example.com -c user.name="M3 Demo" add README.md | Out-Null
git -C $repository -c user.email=m3-demo@example.com -c user.name="M3 Demo" commit -m init | Out-Null
