...
const envelope = (
  ...
  source: { origin: "system" as const, producer: "m4-reliability" },
  payload
});

describe("M4 delivery reliability", () => {
  it("does not create a second deploy when outbox is reclaimed after restart", () => {
    const directory = mkdtempSync(join(tmpdir(), "cimiloop-m4-reliability-"));
    ...
    const initialized = success(
      kernel.execute({
        ...
        source: { origin: "human_cli" as const, producer: "m4-reliability" },
        payload: {
          name: "M4 Reliability",
    ...
    const claimed = store.claimOutbox(now, "2026-09-20T12:01:00.000Z");
    expect(claimed).toBeTruthy();
    if (!claimed) throw new Error("missing outbox");
    store.markOutboxDelivered(claimed.id, now);
    const nextClaim = store.claimOutbox(now, "2026-09-20T12:02:00.000Z");
    expect(nextClaim?.id).not.toBe(claimed.id);

    store.close();
    openStores.pop();
    const reopened = new SqliteProjectStore(databasePath);
    openStores.push(reopened);
    const restarted = new CimiLoopKernel({ store: reopened, now: () => now });
    const operations = restarted.listExternalOperationsByChange(ctx.changeId);
    const deployments = restarted.listDeploymentsByRelease(release.data.release.id);
